...
The source repository is public. To publish the first version privately through GitHub Actions, create a repository secret named `GH_PACKAGES_PAT` containing a personal access token (classic) with `read:packages` and `write:packages`. The workflow uses that secret to create the package and checks that its visibility is **private**. If the secret is removed later, the workflow uses its scoped `GITHUB_TOKEN` only when an existing private package is accessible; otherwise, it packs the library and skips publication.
